...
function y =DynamicModel(y0, tInit, t, delta)
  ...
  comments=("time delta_1 delta_2 delta_3 delta_4")
  Deltas=Deltas.^(1/2)
  csvWrite([t', Deltas'],"Deltas.txt",ascii(9),".","%.7f",comments);
  
  figure(5) 
plot(t,Deltas(1,:),t,Deltas(2,:),t,Deltas(3,:),t,Deltas(4,:));
e=gce();
hl=captions(e.children,['w_1';'w_2';'w_3';'w_4'],'in_lower_right');
xlabel("Time (s)")
ylabel("Control inputs w_i (rad/s)")
xgrid();
endfunction
